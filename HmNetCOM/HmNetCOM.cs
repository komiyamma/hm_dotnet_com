...
namespace HmNetCOM
{
    internal partial class Hm
    {
        ...
        private m_selfDllPath = "";
        private m_selfDllDir = "";

        private static void SetSelfDllPath()
        {
            // ファイル名を取得するためのバッファ
            StringBuilder fileName = new StringBuilder(260); // MAX_PATH (260) を使用するのが一般的ですが、十分な長さを確保します。
            ...
            m_selfDllDir = Path.GetDirectoryName(path);
        }
...
namespace HmNetCOM
{
    ...
    public partial class HmMacroCOMVar {
        private const string HmMacroCOMVarInterface = "77e3df5e-8efa-4c21-886b-5c9df6ae03d3";
    }
}
        ...
        public static partial class Macro
        {
            ...
            public static partial class Exec
            {
                ...
                public static IResult Method(string message_parameter, Delegate delegate_method)
                {
                    ...
                    string methodAssemblyLocation = delegate_method.Method.DeclaringType.Assembly.Location;
                    // 渡されたメソッドが自分自身のdllと異なるのはダメ
                    if (methodAssemblyLocation != GetExecutingAssemblyLocation();
                    {
                    ...
                    {
                        var ret = HmMacroCOMVar.BornMacroScopeMethod(parameter, methodAssemblyLocation, delegate_method.Method.DeclaringType.FullName, delegate_method.Method.Name);
                        if (ret.Result > 0) {
                    ...
                    else if (!delegate_method.Method.IsStatic)
                    {
                        ...
                        string message_no_static = delegate_method.Method.DeclaringType.FullName + "." + delegate_method.Method.Name + " is not 'STATIC' in " + methodAssemblyLocation;
                    ...
                    {
                        string message_no_public = delegate_method.Method.DeclaringType.FullName + "." + delegate_method.Method.Name + " is not 'PUBLIC' in " + methodAssemblyLocation;
                    ...
                    string message_missing = delegate_method.Method.DeclaringType.FullName + "." + delegate_method.Method.Name + "is 'MISSING' access in " + methodAssemblyLocation;
